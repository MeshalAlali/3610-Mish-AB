...
function follow_wall()
    ...
    Kp = 0.0055;  % |err| max ~1 -> |control| max ~2
    Ki = 0.00005;
    Kd = 0.00000; % D term blows up on noisy ultrasonic + tiny dt
    ...
    sideTarget = 350; % wall target (raw units, ~7.5 cm)
    cylinderDetect = 500; % cylinder detect distance (left sensor)

    % % DRIVE TO CYLINDER
    % drive(9, 9, 0.03); % wake motors
    % while true
    %     if nb.ultrasonicRead2() <= cylinderDetect % cylinder in range (left sensor, raw)
    %         break
    %     end
    %     drive(9, 9); % forward
    % end
...
%% ----------------- Turn 90 Right ------------------ %%
function turn90(), drive(10, -12, 0.6); end
%%  Test  %%
turn90()

%% ------------------- Turn Right ------------------- %%
function turn_right(), drive(9, 0, 0.4); end
%%  Test  %%
turn_right()

%% ------------------- Turn Left -------------------- %%
function turn_left(), drive(0, 9, 0.4); end
%%  Test  %%
turn_left()
