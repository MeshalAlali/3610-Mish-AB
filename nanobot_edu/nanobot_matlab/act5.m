...
mOffScale = 1.0; % Start with 1 so both motors have same duty cycle.
...
whiteThresh = max(minVals) + 0.10*(max(maxVals) - max(minVals)); % Max value detected for all white
...
while (toc < 5)  % Adjust me if you want to stop your line following 
    ...
    error = -(sum(min(max(calibratedVals,0),1) .* [-3 -2 -1 1 2 3]) / (sum(min(max(calibratedVals,0),1)) + eps)); % Designing this error term can sometimes be just as 
    ...
    derivative = (error - prevError) / (dt + eps);
